...
export default function CustomInjectionMoldingPage() {
  return (
    <>
      ...
      >
        <Link href="/contact" className="mt-8 inline-flex items-center gap-2 rounded-md bg-white px-5 py-3 text-sm font-semibold text-graphite-950 transition hover:bg-electric-400">
          提交图纸或样品需求
        ...
        <div className="section-shell">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {steps.map(({ icon: Icon, title, text }, index) => (
              <Reveal key={title} delay={index * 0.05} className="glass-panel rounded-lg p-6">
                <div className="flex items-center gap-4">
                  <span className="grid h-11 w-11 place-items-center rounded-md bg-electric-400/15">
                    <Icon className="h-5 w-5 text-electric-400" />
                  </span>
                  <span className="text-xs uppercase tracking-[0.22em] text-steel-500">{String(index + 1).padStart(2, "0")}</span>
                </div>
      ...
      <section className="border-y border-white/10 bg-graphite-900 py-20">
        <div className="section-shell grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {["塑料平垫/垫片非标规格", "汽车塑料配套件", "电子电器结构件与外观件", "宠物用品塑料配件", "家具脚垫、管塞、连接件", "颜色、材料、包装定制"].map((item) => (
              <div key={item} className="rounded-lg border border-white/10 bg-white/[0.045] p-5 text-sm font-semibold text-white">
                {item}
              </div>
            ))}
          </div>
