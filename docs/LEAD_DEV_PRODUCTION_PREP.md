...
`DATABASE_URL` remains the production runtime connection string generated by Vercel/Neon. `DATABASE_URL_UNPOOLED` is the direct connection string used for Prisma schema push and one-time SQLite to PostgreSQL migration.

Follow-up history is stored in `FollowUpRecord`. The existing quick-status fields on `Lead` (`lastContactedAt`, `followUpAt`, `followUpMethod`, `followUpStatus`, `followUpNote`, `repliedAt`, `hasFollowedUp`) remain for dashboard and current-state summaries. During migration, any non-empty legacy follow-up fields are converted into at most one stable `legacy-follow-up-<leadId>` record per lead, so rerunning the migration does not duplicate history.
